...
def znajdz_adres_dostawy(linie):
    ...
    # wszystko w jednej linii ze spacjami
    adres = " ".join(selected)
...
def utworz_zbiorczy_pdf(summary_path, df_summary, total_netto, total_palety):
    ...
    for rec in records:
        ...
        texts_lines = [
            [str(zlec)],                        # ZLECENIE
            ["" if mp_val is None else str(mp_val)],  # Ilość palet (opis z Excela)
