...
@dataclass
class OrderData:
    """Dane o jednym zamówieniu wyciągnięte z PDF."""
    order_number: str
...
def init_font() -> str:
    """Rejestruje czcionkę z polskimi znakami z lokalnego pliku TTF."""
    try:
...
def get_logo_path() -> Optional[str]:
    """Zwraca pełną ścieżkę do logo, jeśli istnieje."""
    try:
        base_dir = os.path.dirname(os.path.abspath(__file__))
    except NameError:
        base_dir = os.getcwd()
...
# ============================================================
# FUNKCJE POMOCNICZE – PARSOWANIE TEKSTU
# ============================================================

def parse_float(text: Optional[str]) -> Optional[float]:
    """Czyści tekst liczbowy i zwraca float albo None."""
    if text is None:
        return None
...
def find_net_weight_summary(lines: List[str]) -> Optional[float]:
    """Szuka 'Całkowita waga netto / Total Net Weight' w podsumowaniu."""
...
def find_delivery_address(lines: List[str]) -> Optional[str]:
    """Wyciąga adres dostawy z bloku 'Adresat/Consignee' ... 'Adres nabywcy'."""
    start = None
    end = None
...
class PackingListExtractor:
    """Wyciąga dane o zamówieniach z PDF."""
    ...
    def extract(self) -> Tuple[Dict[str, OrderData], Set[int]]:
        """
        Zwraca:
        - słownik {numer_zamówienia: OrderData}
        - zbiór indeksów stron z oryginalnego PDF, które trzeba zachować
        """
        ...
        with pdfplumber.open(self.pdf_path) as pdf:
            ...
            # mapowanie: nr zamówienia -> set indeksów stron
            order_pages: Dict[str, Set[int]] = {nr: set() for nr in self.order_numbers}

            # znajdź strony z numerami zamówień
            ...
            # dla każdego zamówienia wyciągamy dane
            for nr in self.order_numbers:
    ...
    def _extract_net_weight(
        ...
        """Szuka wagi netto po stronach danego zamówienia + ewentualnie następnej stronie."""
        ...
        # fallback – kolejna strona po każdej stronie zamówienia
        for idx in page_indices:
            next_idx = idx + 1
            if next_idx < len(pages_lines):
...
class ExcelExporter:
    """Zapisuje dane zamówień do Excela."""
...
class SummaryPdfBuilder:
    """Buduje zbiorczy list przewozowy (strony landscape z tabelą)."""
    ...
    def build(self, summary_path: str, results: Dict[str, OrderData],
        ...
        - landscape A4,
        - tytuł na środku,
        - po lewej adres Kersia,
        - po prawej data,
        - pod datą większe logo,
        - tabela z zamówieniami,
        - pod tabelą wiersz z 'CAŁKOWITA WAGA NETTO (kg)',
        - stopka z podpisami i kropkami do wypełnienia niżej.
        """
        c = canvas.Canvas(summary_path, pagesize=landscape(A4))
        width, height = landscape(A4)

        def draw_header(title_suffix: str = "") -> float:
            """Nagłówek z tytułem, nadawcą, datą i logo."""
        ...
        def draw_footer() -> None:
            """Stopka: podpisy + kropki do wypełnienia wiersz niżej."""
            c.setFont(self.font_name, 10)
            y = 40
            left_x = width * 0.25
            right_x = width * 0.60
        ...
        for nr in order_numbers:
            ...
            # zostaw miejsce na stopkę + wiersz sumy
            if y - row_h < 70:
                draw_footer()
                y = new_page()
                draw_row(y, header_h, header_texts, header_bold)
        ...
        # wiersz z CAŁKOWITĄ WAGĄ NETTO pod tabelą
        y -= 10
